class MLPlay:
    ...
    def __init__(self):
        ...
        self.serve_step = [
             ...
             "MOVE_LEFT", "MOVE_LEFT",
                           "SERVE_TO_RIGHT"]
        # self.serve_step = ["MOVE_LEFT", "MOVE_LEFT", "MOVE_LEFT", "MOVE_LEFT", "MOVE_LEFT",
    ...
    def update(self, scene_info):
        ...
        #     command = "SERVE_TO_LEFT"
            ...
            print("------------------------------------------------------------")
            # if self.move == "down_left" and self.location[1] >= 340 and self.location[0] < scene_info["platform"][0] + 40 + 14:
            #     return "MOVE_LEFT"
            # elif self.move == "down_right" and self.location[1] >= 340 and self.location[0] + 5 > scene_info["platform"][0] - 14:
            #     return "MOVE_RIGHT"
    ...
    def movement(self, scene_info):
        ...
        if scene_info["platform"][0] + 30 < self.location[0]:
            command = "MOVE_RIGHT"
            print("platfrom MOVE_RIGHT", scene_info["platform"])

        elif scene_info["platform"][0] + 10 > self.location[0] + 5:
            command = "MOVE_LEFT"
            print("platfrom MOVE_LEFT", scene_info["platform"])

        else:
            command = "NONE"
            print("platfrom NONE", scene_info["platform"])
    ...
    def get_direction(self):
        ...
        vector_x = self.location_current[0] - self.location_last[0]
        vector_y = self.location_current[1] - self.location_last[1]
        if abs(vector_x) == 7 or abs(vector_y) == 7:
            self.speed = 7
        elif abs(vector_x) == 10 or abs(vector_y) == 10:
            self.speed = 10
        ...
        if self.move == "up_left":
            for brick in self.consider_bricks:
                if self.brick_left(brick) < self.ball_left() and self.brick_top(brick) < self.ball_top():
                    # check left down and right up coner
                    if (1 * (self.brick_left(brick) - self.ball_left()) + self.ball_bottom() <= self.brick_bottom(brick)\
                        and 1 * (self.brick_right(brick) - self.ball_left()) + self.ball_bottom() >= self.brick_top(brick))\
                        or (1 * (self.brick_left(brick) - self.ball_right()) + self.ball_top() <= self.brick_bottom(brick)\
                         and 1 * (self.brick_right(brick) - self.ball_right()) + self.ball_top() >= self.brick_top(brick)):
            ...
            for brick in self.consider_bricks:
                if self.brick_right(brick) > self.ball_right() and self.brick_top(brick) < self.ball_top():
                    # check left up and right down coner
                    if (-1 * (self.brick_left(brick) - self.ball_left()) + self.ball_top() >= self.brick_top(brick)\
                        and - 1 * (self.brick_right(brick) - self.ball_left()) + self.ball_top() <= self.brick_bottom(brick))\
                        or (-1 * (self.brick_left(brick) - self.ball_right()) + self.ball_bottom() >= self.brick_top(brick)\
                        and -1 * (self.brick_right(brick) - self.ball_right()) + self.ball_bottom() <= self.brick_bottom(brick)):
                        possible_bricks.append(brick)

        elif self.move == "down_left":
            for brick in self.consider_bricks:
                if self.brick_left(brick) < self.ball_left() and self.brick_bottom(brick) > self.ball_bottom():
                    # check left up and right down coner
                        if (-1 * (self.brick_left(brick) - self.ball_left()) + self.ball_top() >= self.brick_top(brick)\
                            and - 1 * (self.brick_right(brick) - self.ball_left()) + self.ball_top() <= self.brick_bottom(brick))\
                            or (-1 * (self.brick_left(brick) - self.ball_right()) + self.ball_bottom() >= self.brick_top(brick)\
            ...
            for brick in self.consider_bricks:
                if self.brick_right(brick) > self.ball_right() and self.brick_bottom(brick) > self.ball_bottom():
                    # check left down and right up coner
                    if (1 * (self.brick_left(brick) - self.ball_left()) + self.ball_bottom() <= self.brick_bottom(brick)\
                        and 1 * (self.brick_right(brick) - self.ball_left()) + self.ball_bottom() >= self.brick_top(brick))\
                        or (1 * (self.brick_left(brick) - self.ball_right()) + self.ball_top() <= self.brick_bottom(brick)\
            ...
            if self.move == "up_left":
                for brick in possible_bricks:
                    if self.power_of_distance(self.brick_right(brick),  self.ball_left(), self.brick_bottom(brick), self.ball_top()) <= min_distance:
                        closest_brick = brick
                        if brick in self.consider_bricks_hard:
                            self.consider_bricks_hard.remove(brick)
                        else:
    ...
    def hit_vertical(self, hit_x):
        ...
        if self.move == "up_left":
            ...
            if h_distance % self.speed == 0:
                ...
                self.location[1] += (int(h_distance /
                                         self.speed)+1) * self.speed
                if self.location[1] > 400 - 5:
                    self.location[1] = 400 - 5
    ...
    def hit_horizontal(self, hit_y):
        ...
        if self.move == "up_left":
            ...
            if v_distance % self.speed == 0:
                ...
                self.location[0] += (int(v_distance /
                                         self.speed)+1) * self.speed
                if self.location[0] > 200 - 5:
                    self.location[0] = 200 - 5
